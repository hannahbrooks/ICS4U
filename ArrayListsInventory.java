...
public class GameInterface {
    ...
    public static void main (String[] args) {
        DisplayScore ms = new DisplayScore(30); // An instance for DisplayScore.
 ...
 * 	addToList - Add a score from the file to the array based on insertion sort.
...
public class DisplayScore {
    ...
    private static ArrayList<GameEntry> scores; // Array to store the scores.
    ...
    public DisplayScore(int LENGTH) {
        scores = new ArrayList<GameEntry>(LENGTH); // Initializes array size.
        count = 0;
    }
    
    public static void fileReadMethod(File f) {
        try{
            BufferedReader read = new BufferedReader(new FileReader(f));
            String line; // Where each score is stored.
            
            while ((line = read.readLine()) != null){
                addToList(new GameEntry(line)); // Calling add method to add scores into the array in order of score.
            }
    ...
    public static void addToList(GameEntry newEntry) {
        
        if (count == 30 ) { // When the array has reached the max.
            if (newEntry.getScore() > scores.get(29).getScore())
                return; // When the score doesn't make into the array.
            else
                scores.set(29, newEntry); // If the scores makes it in.
        }
        else
            scores.add(newEntry);
        
        GameEntry temp;
        for (int i = 1; i < count +1; i++){ // To go through the entire array.
            for (int j = i; j > 0 && scores.get(j-1).getScore() > scores.get(j).getScore(); j--){ // To check the values in front of the newEntry.
                // Swap.
                temp = scores.get(j);
    ...
    public void remove() {
        ...
        while (i < count) {
            if (scores.get(i).getName().equals(playerName)) {
                // Delete the score.
                for (int j = i; j < count; j++) {
                    if (j+1 < count)
                        scores.set(j, scores.get(j+1)); // Remove the element by moving each element up.
                    else
                        scores.set(j,null); // Last on the list is null.
                }
                deleted++;
    ...
    public void sortByScore() {
        ...
        ListIterator<GameEntry> it = scores.listIterator();
        
        do {
            int min = it.nextIndex();  // Smallest value of the array.
            GameEntry temp;  // Used for swapping.
            
            // Find the smallest value.
            for (int j = it.nextIndex()+1; j < count; j++) {
                if (scores.get(it.nextIndex()).getScore() > scores.get(j).getScore())
            ...
            if (scores.get(min).getScore() > scores.get(it.nextIndex()).getScore()) {
                // Swap.
                temp = scores.get(it.nextIndex());
                scores.set(it.nextIndex(), scores.get(min));
                scores.set(min, temp);
            }
            it.next();
        } while (it.hasNext());
    ...
    public void sortByName() {
        for (int i = 0; i < count; i++) {
            ...
            for (int j = 0; j < count-1; j++) {
                ...
                if (scores.get(j).getName().compareToIgnoreCase(scores.get(j+1).getName()) > 0) {
                    temp = scores.get(j);
                    scores.set(j, scores.get(j+1));
                    scores.set(j+1, temp);
                }
    ...
    public void sortByLevel() {
        ...
        for (int i=1; i< count; ++i) {
            ...
            while (j>=0 && scores.get(j).getScore() > scores.get(i).getScore() && scores.get(j).getLevel() == scores.get(i).getLevel()) {
                scores.set(j+1,  scores.get(j)); // Swap them.
                j--;
            }
            scores.set(j+1, scores.get(i)); // Fill the one that was just switched.
        }
    }
    ...
    public String toString() {
        ...
        for (int i = 0; i < count; i++) {
            if (i > 0)
                s += "\n\n";
            s += scores.get(i);  // Calling the other toString().
        }
        return s;
    }
}
 ...
 * It also has a toString method used to return the information of each score as well as a
 * equals method to return the equality of two Strings.
 *
 *************************************************************************************************/
